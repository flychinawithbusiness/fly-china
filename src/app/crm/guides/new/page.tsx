...
export default function NewGuidePage() {
  ...
  const [form, setForm] = useState({
    name: "", phone: "", email: "",
    language: "Bangla, English, Chinese",
    speciality: "",
  })
  ...
  return (
    <div style={{ maxWidth: 600 }}>
      <h1 style={{ fontFamily: "var(--font-display)", fontStyle: "italic", fontWeight: 700, fontSize: "2rem", color: "#1C3A6B", margin: "0 0 2rem" }}>Add Guide</h1>
      <form onSubmit={handleSubmit}>
        <div style={{ background: "white", borderRadius: 20, padding: "2rem", border: "1px solid rgba(28,58,107,0.07)", boxShadow: "0 4px 16px rgba(28,58,107,0.06)", display: "flex", flexDirection: "column", gap: "1.25rem" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
            <div><label style={labelStyle}>Full Name *</label><input value={form.name} onChange={e => set("name", e.target.value)} required placeholder="Guide name" style={inputStyle} /></div>
          ...
          <div><label style={labelStyle}>Languages</label><input value={form.language} onChange={e => set("language", e.target.value)} placeholder="Bangla, English, Chinese" style={inputStyle} /></div>
          <div><label style={labelStyle}>Speciality</label><input value={form.speciality} onChange={e => set("speciality", e.target.value)} placeholder="Electronics, Fashion markets..." style={inputStyle} /></div>
          <div style={{ display: "flex", gap: "0.75rem" }}>
            <button type="submit" disabled={loading} style={{ background: "linear-gradient(135deg, #1C3A6B, #2A5099)", color: "white", border: "none", borderRadius: 12, padding: "0.8rem 2rem", fontFamily: "var(--font-body)", fontWeight: 700, fontSize: "0.875rem", cursor: "pointer" }}>{loading ? "Saving..." : "Save Guide"}</button>
