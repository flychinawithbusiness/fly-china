...
export default function EditHotelPage() {
  ...
  const [form, setForm] = useState({ name: "", location: "", address: "", phone: "", stars: 3, notes: "", isActive: true })

  useEffect(() => {
    fetch(`/api/crm/hotels/${id}`).then(r => r.json()).then(data => {
      setForm({ name: data.name || "", location: data.location || "", address: data.address || "", phone: data.phone || "", stars: data.stars || 3, notes: data.notes || "", isActive: data.isActive ?? true })
    })
  }, [id])
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    setLoading(true)
    await fetch(`/api/crm/hotels/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) })
    setLoading(false)
  ...
  return (
    <div style={{ maxWidth: 600 }}>
      <h1 style={{ fontFamily: "var(--font-display)", fontStyle: "italic", fontWeight: 700, fontSize: "2rem", color: "#1C3A6B", margin: "0 0 2rem" }}>Edit Hotel</h1>
      <form onSubmit={handleSubmit}>
        <div style={{ background: "white", borderRadius: 20, padding: "2rem", border: "1px solid rgba(28,58,107,0.07)", boxShadow: "0 4px 16px rgba(28,58,107,0.06)", display: "flex", flexDirection: "column", gap: "1.25rem" }}>
          <div><label style={labelStyle}>Hotel Name *</label><input value={form.name} onChange={e => set("name", e.target.value)} required style={inputStyle} /></div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
            ...
            <label htmlFor="isActive" style={{ fontSize: "0.875rem", color: "#374151", fontFamily: "var(--font-body)", cursor: "pointer" }}>Active</label>
          </div>
          <div style={{ display: "flex", gap: "0.75rem" }}>
            <button type="submit" disabled={loading} style={{ background: "linear-gradient(135deg, #1C3A6B, #2A5099)", color: "white", border: "none", borderRadius: 12, padding: "0.8rem 2rem", fontFamily: "var(--font-body)", fontWeight: 700, fontSize: "0.875rem", cursor: "pointer" }}>{loading ? "Saving..." : "Update Hotel"}</button>
