...
export default function EditCustomerPage() {
  ...
  const [form, setForm] = useState({
    ...
    passportNumber: "", address: "", notes: "",
  })

  useEffect(() => {
    fetch(`/api/crm/customers/${id}`).then(r => r.json()).then(data => {
      setForm({
        name: data.name || "", phone: data.phone || "", email: data.email || "",
        nid: data.nid || "", passportNumber: data.passportNumber || "",
  ...
  return (
    <div style={{ maxWidth: 700 }}>
      <h1 style={{ fontFamily: "var(--font-display)", fontStyle: "italic", fontWeight: 700, fontSize: "2rem", color: "#1C3A6B", margin: "0 0 2rem" }}>Edit Customer</h1>
      <form onSubmit={handleSubmit}>
        <div style={{ background: "white", borderRadius: 20, padding: "2rem", border: "1px solid rgba(28,58,107,0.07)", boxShadow: "0 4px 16px rgba(28,58,107,0.06)", display: "flex", flexDirection: "column", gap: "1.25rem" }}>
          ...
          <div style={{ display: "flex", gap: "0.75rem" }}>
            <button type="submit" disabled={loading} style={{ background: "linear-gradient(135deg, #1C3A6B, #2A5099)", color: "white", border: "none", borderRadius: 12, padding: "0.8rem 2rem", fontFamily: "var(--font-body)", fontWeight: 700, fontSize: "0.875rem", cursor: "pointer" }}>{loading ? "Saving..." : "Update Customer"}</button>
